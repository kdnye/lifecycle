...
| `MIGRATE_ON_STARTUP` | No | `false` | Deprecated for production. Run migrations in CI/CD or a dedicated job before deploy. |
...
3. Run `flask db upgrade` as a standalone CI/CD step (or Cloud Run Job) before deployment.
